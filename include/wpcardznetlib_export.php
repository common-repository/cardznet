...
if (!class_exists('WPCardzNetLibExportAdminClass'))
{

	class WPCardzNetLibExportAdminClass // Define class
	{
		var $myDBaseObj;
		var $fieldNames;
		...
		function header($content)
		{
				...
				|| $this->myDBaseObj->isDbgOptionSet('Dev_ShowDBOutput') )
			{
				if (!isset($this->doneFirstHeader)) 
					WPCardzNetLibEscapingClass::Safe_EchoHTML("SQL Debug Output Enabled - HTML Headers output as text<br>\n");
				WPCardzNetLibEscapingClass::Safe_EchoHTML($content."<br>\n");
		...
		function exportDB($dbList, $exportHTML = false)
		{
			...
			WPCardzNetLibEscapingClass::Safe_EchoScript($exportData);
		}
		...
		function GetExport($dbList, $exportHTML = false)
		{
			...
			foreach($dbList as $dbEntry)
			{
				...
				foreach ($keysList as $key => $unused)
				{
					...
					$option = WPCardzNetLibMigratePHPClass::Safe_str_replace("\r\n",",",$option);
					$option = WPCardzNetLibMigratePHPClass::Safe_str_replace("\r",",",$option);
