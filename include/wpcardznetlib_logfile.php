...
if (!class_exists('WPCardzNetLibLogFileClass'))
{
	class WPCardzNetLibLogFileClass // Define class
	{
		const ForReading = 1;
		const ForWriting = 2;
		...
		function __construct($LogsFolderPath = '')
		{
			...
			if (WPCardzNetLibMigratePHPClass::Safe_substr($LogsFolderPath, -1) === '/')
			{
				$LogsFolderPath = WPCardzNetLibMigratePHPClass::Safe_substr($LogsFolderPath, 0, -1);
			}

			if (!is_dir($LogsFolderPath))
			{
				mkdir($LogsFolderPath, WPCARDZNETLIB_LOGFOLDER_PERMS, true);
			}
		...
		{
			$Filepath = WPCardzNetLibMigratePHPClass::Safe_str_replace("\\", "/", $Filename);

			for ($i = 1; $i <= 2; $i++)
			{
				$firstChar = WPCardzNetLibMigratePHPClass::Safe_substr($Filepath, 0, 1);
				if (WPCardzNetLibMigratePHPClass::Safe_strpos($Filepath, ':') || ($firstChar == '/'))
				{
				...
				else if ($i == 1)
				{
					...
					if (WPCardzNetLibMigratePHPClass::Safe_strlen($this->LogsFolderPath) > 0)
		...
		{			
			...
			$LogStamp .= 'Log Length: '.WPCardzNetLibMigratePHPClass::Safe_strlen($LogLine)."\n";
			$LogStamp .= "\n";
		...
		static function LogToFileAbs($Filepath, $LogLine, $OpenMode = self::ForAppending, $LogHeader = '')
		{
			...
			if ($logFile != 0)
			{
				...
				if ($LogHeader !== '')
				{
					...
					if (ftell($logFile)	=== 0)
						fwrite($logFile, $LogHeader, WPCardzNetLibMigratePHPClass::Safe_strlen($LogHeader));
				}

				fwrite($logFile, $LogLine, WPCardzNetLibMigratePHPClass::Safe_strlen($LogLine));
				fclose($logFile);
			...
			{
				WPCardzNetLibEscapingClass::Safe_EchoHTML("Error writing to ".$Filepath."<br>\n");
				$rtnStatus = false;
			}
		...
		function DumpToFile($Filename, $dataId, $dataToDump)
		{
			...
			$dataLen = WPCardzNetLibMigratePHPClass::Safe_strlen($dataToDump);
			...
			for ($i=0;;$i++)
			{
				...
				$nextChar = WPCardzNetLibMigratePHPClass::Safe_substr($dataToDump, $i, 1);
				if ($i < $dataLen)
				{
		...
		function AddToTestLog($LogLine)
		{
			...
			self::LogToFileAbs($Filepath, 'Log Time/Date:'.WPCardzNetLibDBaseClass::GetLocalDateTime()."\n");
			//self::LogToFileAbs($Filepath, 'Request URL:'.WPCardzNetLibUtilsClass::GetHTTPTextElem('server', 'REQUEST_URI')."\n");
