...
if (!class_exists('WPCardzNetLibTableClass')) 
{
	if (!defined('WPCARDZNETLIB_EVENTS_PER_PAGE'))
		define('WPCARDZNETLIB_EVENTS_PER_PAGE', 20);
	
	if (!defined('WPCARDZNETLIB_DISABLE_POSTCONTROLS')) 
		define('WPCARDZNETLIB_DISABLE_POSTCONTROLS', true);
	
	class WPCardzNetLibTableUtilsClass // Define class
	{
		static function GetCurrentPage()
		{ 
			$currentPage = WPCardzNetLibUtilsClass::GetHTTPInteger('request', 'paged', 1);
			$currentPage = WPCardzNetLibUtilsClass::GetHTTPInteger('get', 'paged', $currentPage);
			return $currentPage;
		}

	}
	
	class WPCardzNetLibTableClass // Define class
	{
		...
		const WPCARDZNETLIB_EVENTS_UNPAGED = -1;
		...
		var $showOptionsID = 0;
		var $hiddenRowClass  = 'wpcardznetlib_hiderow';
		var $visibleRowClass = '';
		...
		function __construct($newTableType = self::TABLETYPE_HTML) //constructor
		{
			...
			switch ($this->tableType)
			{
				...
				default:
					...
					WPCardzNetLibEscapingClass::Safe_EchoHTML("<strong><br>Invalid table type ($newTableType) ".get_class($this)." class<br></strong>\n");
					die;
					break;
			}
			...
			$this->moreText = __('Show', 'wpcardznet');
			$this->lessText = __('Hide', 'wpcardznet');
		...
		function SetRowsPerPage($rowsPerPage)
		{
			...
			$this->currentPage = WPCardzNetLibTableUtilsClass::GetCurrentPage();			
		}
		...
		{
			WPCardzNetLibUtilsClass::UndefinedFuncCallError($this, 'GetMainRowsDefinition');
		}
		...
		function EnableListHeaders()
		{
			...
			if (isset($this->bulkActions))
			{
				...
				$columns = array_merge(array('eventCb' => '<input name="checkall" id="checkall" type="checkbox"  onClick="WPCardzNetLib_updateCheckboxes(this)" />'), $columns); 
			}
		...
		function AddCheckBoxToTable($result, $columnDef, $checked=false, $col=0, $checkedValue='checked', $label='', $newRow = false)
		{
			...
			if (WPCardzNetLibMigratePHPClass::Safe_substr($inputName, -2) != '[]')
			{
		...
		function GetInputHTML($inputName, $maxlength, $value, $extraParams='')
		{
			...
			{
				if (WPCardzNetLibMigratePHPClass::Safe_strpos($extraParams, 'class="') !== false)
				{
					$extraParams = WPCardzNetLibMigratePHPClass::Safe_str_replace('class="', 'class="wpcardznetlib_PostVal ', $extraParams);
				}
				else
				{
					$extraParams .= ' class="wpcardznetlib_PostVal"';					
				}
			}
			else
			{
			...
			if ($this->allowHiddenTags)
			{
				...
				if ($this->tableUsesSerializedPost)
				{
					$baseParams = 'class="wpcardznetlib_PostVal"';					
				}
				else
				{
		...
		function AddSelectToTable($result, $columnDef, $options, $value='', $col=0, $newRow = false)
		{
			...
			if ($this->tableUsesSerializedPost)
			{
				$params = ' class="wpcardznetlib_PostVal"';
			}
			else
			{
				$params = " name=$inputName";
		...
		function AddShowOrHideButtonToTable($result, $tableId, $rowId, $content, $col=0, $newRow = false)
		{
			...
			$content = '<a id="'.$moreName.'" class="button-secondary" onClick="WPCardzNetLib_HideOrShowRows(\''.$moreName.'\', \''.$rowId.'\')">'.$content.'</a>';
			$this->AddToTable($result, $content, $col, $newRow);
		}
		...
		function AddToTable($result, $content, $col=0, $newRow = false)
		{
			...
			{
			if (!isset($content) || (WPCardzNetLibMigratePHPClass::Safe_strlen($content) == 0)) return;
			}
		...
		{
			return 'wpcardznetlib_ClickHeader(this)';
		}
		...
		function Output_ColHeader()
		{
			...
			foreach ($this->mergedColumns as $id => $html_col)
			{
				if ($addSeparator)
				{
					...
					WPCardzNetLibEscapingClass::Safe_EchoHTML($html_cell."\n");
				}
		...
		function ColumnHeaders($atTop = true)
		{
			...
			if ($atTop)
			{
				...
				WPCardzNetLibEscapingClass::Safe_EchoHTML("<thead>\n");
			...
			else
			{
				...
				WPCardzNetLibEscapingClass::Safe_EchoHTML("<tfoot>\n");
				WPCardzNetLibEscapingClass::Safe_EchoHTML("<tr>\n");
				$this->Output_ColHeader();
				WPCardzNetLibEscapingClass::Safe_EchoHTML("</tr>\n");
				WPCardzNetLibEscapingClass::Safe_EchoHTML("</tfoot>\n");
				WPCardzNetLibEscapingClass::Safe_EchoHTML("<tbody>\n");
			}
		}
		...
		function Header()
		{
			switch ($this->tableType)
			{
				case self::TABLETYPE_HTML:
					if ($this->divClass)
						WPCardzNetLibEscapingClass::Safe_EchoHTML("<div class=$this->divClass>\n");
					...
					WPCardzNetLibEscapingClass::Safe_EchoHTML($html_tab);
		...
		function Footer()
		{
			...
			{
				case self::TABLETYPE_HTML:
					WPCardzNetLibEscapingClass::Safe_EchoHTML("</tbody></table>\n");
					if ($this->divClass)
		...
		{	
			...
			$bulkActions = __('Bulk Actions', 'wpcardznet');
			
			$onclickParam = "onclick=\"return WPCardzNetLib_confirmBulkAction(this, '$buttonId')\"";
			...
			$output .= "</select>\n"; 
			$output .= "<input type='submit' name='doaction$ctrlPosn' id='doaction$ctrlPosn' $onclickParam class='button-secondary action' value=".__('Apply', 'wpcardznet')."  />\n";
			$output .= "</div>\n"; 
...
lessText = '{$this->lessText}';
			...
			";
			WPCardzNetLibEscapingClass::Safe_EchoScript($js_more);
		}
...
var confirmActionsArray = new Array(";
			...
			WPCardzNetLibEscapingClass::Safe_EchoScript($html_bulkActions);
		}
		
		function GetCurrentURL() 
		{			
			$currentURL = WPCardzNetLibUtilsClass::GetPageURL();
		...
		{			
			...
			$page_links[] = sprintf( "<a class='%s' title='%s' %s>%s</a>",
				...
				$disable_first === '' ? __('Go to the first page', 'wpcardznet') : '',
			...
			$page_links[] = sprintf( "<a class='%s' title='%s' %s>%s</a>",
				...
				$disable_first === '' ? __('Go to the previous page', 'wpcardznet') : '',
			...
			{
				$onKeyCodeHandler = ' onkeypress="WPCardzNetLib_SubmitOnReturnKey(this, event);" ';
				$html_current_page = sprintf( "<input class='current-page' title='%s' type='text' name='%s' value='%s' $onKeyCodeHandler size='%d' />",
					__( 'Current page', 'wpcardznet'),
					'paged',
					$this->currentPage,
					WPCardzNetLibMigratePHPClass::Safe_strlen( $totalPages )
				);
			}

			$html_total_pages = sprintf( "<span class='total-pages'>%s</span>", number_format_i18n( $totalPages ) );
			$page_links[] = '<span class="paging-input">' . sprintf('%1$s '.__('of', 'wpcardznet').' %2$s', $html_current_page, $html_total_pages ) . '</span>';

			$page_links[] = sprintf( "<a class='%s' title='%s' %s>%s</a>",
				'next-page button' . $disable_last,
				$disable_last === '' ? __('Go to the next page', 'wpcardznet') : '',
			...
			$page_links[] = sprintf( "<a class='%s' title='%s' %s>%s</a>",
				...
				$disable_last === '' ? __('Go to the last page', 'wpcardznet') : '',
		...
		function ShowPageControls($which = 'top')
		{
			...
			if ($pageControls != '') 
			{
				...
				WPCardzNetLibEscapingClass::Safe_EchoHTML($html_bulkActions);
			}
		}
			...
			for ($row = 1; $row <= $this->currRow; $row++)
			{
				...
				switch ($this->tableType)
				{
					case self::TABLETYPE_HTML:
						if (isset($this->rowAttr[$row]) && ($this->rowAttr[$row] != ''))
						{
							...
							WPCardzNetLibEscapingClass::Safe_EchoHTML($html_row);
						}
						else
							WPCardzNetLibEscapingClass::Safe_EchoHTML("<tr>\n");
						break;
				...
				for ($col = 1; $col <= $this->maxCol; $col++)
				{
					...
					switch ($this->tableType)
					{
						case self::TABLETYPE_HTML:
							...
							WPCardzNetLibEscapingClass::Safe_EchoHTML($html_table);
							break;
						case self::TABLETYPE_RTF:
							if ($col > 1) WPCardzNetLibEscapingClass::Safe_EchoAttr('\tab ');
						case self::TABLETYPE_TEXT:
						default:
							break;
					}
					$html_tableContents = isset($this->tableContents[$row][$col]) ? $this->tableContents[$row][$col] : "";
					$html_tableContents = WPCardzNetLibMigratePHPClass::Safe_trim($html_tableContents);
					if (WPCardzNetLibMigratePHPClass::Safe_strlen($html_tableContents) == 0) $html_tableContents = "&nbsp;";
					...
					WPCardzNetLibEscapingClass::Safe_EchoHTML($html_tableContents);
				...
				switch ($this->tableType)
				{
					case self::TABLETYPE_HTML:
						...
						break;
					case self::TABLETYPE_RTF:
						WPCardzNetLibEscapingClass::Safe_EchoAttr('\par '."\n");
						break;
					case self::TABLETYPE_TEXT:
					default:
						WPCardzNetLibEscapingClass::Safe_EchoAttr("\n");
						break;
				}
			}
