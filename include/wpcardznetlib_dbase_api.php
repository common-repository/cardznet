...
require_once WPCARDZNETLIB_INCLUDE_PATH.'wpcardznetlib_utils.php';
require_once WPCARDZNETLIB_INCLUDE_PATH.'wpcardznetlib_dbase_base.php';
require_once WPCARDZNETLIB_INCLUDE_PATH.'wpcardznetlib_logfile.php';

if (!class_exists('WPCardzNetLibDBaseClass'))
{
	if (!defined('WPCARDZNETLIB_EVENTS_PER_PAGE'))
		define('WPCARDZNETLIB_EVENTS_PER_PAGE', 20);

	if (!defined('WPCARDZNETLIB_SESSION_TIMEOUT'))
		define('WPCARDZNETLIB_SESSION_TIMEOUT', 60*60*24);

	define('WPCARDZNETLIB_PLUGINNAME', basename(dirname(dirname(__FILE__))));
	define('WPCARDZNETLIB_FILENAME_COMMSLOG', WPCARDZNETLIB_PLUGINNAME.'.log');

	if (!defined('WPCARDZNETLIB_SITEID_OPTIONID'))
		define('WPCARDZNETLIB_SITEID_OPTIONID', 'OrganisationID');

	if (!defined('WPCARDZNETLIB_LOGOIMAGE_OPTIONID'))
		define('WPCARDZNETLIB_LOGOIMAGE_OPTIONID', 'PayPalLogoImageFile');

	define('WPCARDZNETLIB_SESSIONERR_FALSE', 1);
	define('WPCARDZNETLIB_SESSIONERR_INACTIVE', 2);
	define('WPCARDZNETLIB_SESSIONERR_NOTABLE', 3);
	define('WPCARDZNETLIB_SESSIONERR_NOMATCH', 4);

	class WPCardzNetLibDBaseClass extends WPCardzNetLibGenericDBaseClass // Define class
	{
		...
		const SessionDebugPrefix = 'wpcardznetlib_debug_';
		...
		var	$emailObjClass = 'WPCardzNetLibHTMLEMailAPIClass';
		var	$emailClassFilePath = WPCARDZNETLIB_INCLUDE_PATH.'wpcardznetlib_htmlemail_api.php';
		...
		{
			if (current_user_can(WPCARDZNETLIB_CAPABILITY_DEVUSER))
			{
				$className = get_class($this);
				WPCardzNetLibEscapingClass::Safe_EchoHTML("PurgeDB not defined in $className class <br>\n");
			}
		}
		...
		{
			$this->SetSessionElem(WPCARDZNETLIB_SESSION_VALIDATOR, true);	// Set Element and Write Cache to DB
		}
		...
		static function GetIPAddr()
		{
			...
			if (WPCardzNetLibUtilsClass::IsElementSet('server', 'X-Forwarded-For'))
			{
				$the_ip  = WPCardzNetLibUtilsClass::GetHTTPTextElem('server', 'X-Forwarded-For');
			}
			elseif (WPCardzNetLibUtilsClass::IsElementSet('server', 'HTTP_X_FORWARDED_FOR'))
			{
				$the_ip  = WPCardzNetLibUtilsClass::GetHTTPTextElem('server', 'HTTP_X_FORWARDED_FOR');
			}
			else
			{
				$the_ip  = WPCardzNetLibUtilsClass::GetHTTPTextElem('server', 'REMOTE_ADDR');
			}
		...
		function IsButtonClicked($buttonID)
		{
			...
			$rtnVal = (WPCardzNetLibUtilsClass::IsElementSet('request', $normButtonID) || WPCardzNetLibUtilsClass::IsElementSet('request', $normButtonID.'_x'));
			return $rtnVal;
		}
		...
		function getDebugFlagsArray()
		{
			...
			$len = WPCardzNetLibMigratePHPClass::Safe_strlen(self::SessionDebugPrefix);
			$sessionKeys = array_keys($this->GetSession());
			foreach ($sessionKeys as $key)
			{
				if (WPCardzNetLibMigratePHPClass::Safe_substr($key, 0, $len) != self::SessionDebugPrefix)
					continue;
				$debugFlagsArray[] = $key;
			}
		...
		function WPNonceField($referer = '', $name = '_wpnonce', $echoOut = true)
		{
			...
			if ($echoOut) WPCardzNetLibEscapingClass::Safe_EchoHTML($html_nonce);
			return $html_nonce;
		}
		...
		function ActionButtonHTML($buttonText, $caller, $domainId, $buttonClass, $elementId = 0, $buttonAction = '', $extraParams = '', $target = '')
		{
			...
			$page = WPCardzNetLibUtilsClass::GetHTTPTextElem('get', 'page');
		...
		function URLsToAnchor($page)
		{
			...
			if ($noOfMatches !== false)
			{
				...
				{ 
					$entry = $matches[0][$i];
					$url = $matches[2][$i];
					$txt = WPCardzNetLibMigratePHPClass::Safe_trim($matches[1][$i]);
					
					$page = WPCardzNetLibMigratePHPClass::Safe_str_replace($entry, '<a target="_blank" href="'.$url.'">'.$txt.'</a>', $page);
				}
			}
		...
		function get_pluginInfo($att = '')
		{
			if (!isset($this->pluginInfo))
			{
				...
				else
				{
					...
					for ($i = 0; ($i < 10) && WPCardzNetLibMigratePHPClass::Safe_strpos($basename, '/'); $i++)
		...
		function ShowDebugModes()
		{
			...
			{
				$html_dbgmsg .= '<strong>'.__('Session Debug Modes', 'wpcardznet').':</strong> ';
				$comma = '';
				foreach ($debugFlagsArray as $debugMode)
				{
					$debugMode = $comma.WPCardzNetLibMigratePHPClass::Safe_str_replace(self::SessionDebugPrefix, '', $debugMode);
			...
			if (defined('WPCARDZNETLIB_BLOCK_HTTPS'))
			{
				$html_dbgmsg .= '<strong>'.__('SSL over HTTP', 'wpcardznet').":</strong> Blocked<br>\n";
			}

			WPCardzNetLibEscapingClass::Safe_EchoHTML($html_dbgmsg);
		...
		function InTestMode()
		{
			if (!$this->IsSessionElemSet('wpcardznetlib_debug_test')) return false;

			if (!function_exists('wp_get_current_user')) return false;

			return current_user_can(WPCARDZNETLIB_CAPABILITY_DEVUSER);
		}
		...
		{
			$rtnVal = WPCardzNetLibHTTPIO::GetRequestedString($paramId, $defaultVal);
		...
		function getOptionsFromDB()
		{
			...
			{
				WPCardzNetLibEscapingClass::Safe_EchoHTML('CfgOptionsID must be defined<br>');
				exit;
			}

			if (!isset($this->opts['DbgOptionsID']))
			{
				WPCardzNetLibEscapingClass::Safe_EchoHTML('DbgOptionsID must be defined<br>');
				exit;
			}

			// Get current values from MySQL
			$currOptions = $this->ReadSettings($this->opts['CfgOptionsID']);
			if (WPCardzNetLibUtilsClass::IsElementSet('get', 'nodbg'))
			{
		...
		function getOptions($childOptions = array())
		{
			...
			$ourOptions = array(
				...
				'LogsFolderPath' => 'logs',
				'PageLength' => WPCARDZNETLIB_EVENTS_PER_PAGE,
		...
		function dev_ShowTrolley()
		{
			...
			if ($this->isDbgOptionSet('Dev_ShowTrolley') || $this->IsSessionElemSet('wpcardznetlib_debug_trolley'))
			{
				if ($this->getDbgOption('Dev_ShowCallStack') || $this->IsSessionElemSet('wpcardznetlib_debug_stack'))
				{
		...
		function GetRowsPerPage()
		{
			if (isset($this->adminOptions['PageLength']))
				...
				$rowsPerPage = WPCARDZNETLIB_EVENTS_PER_PAGE;
		...
		{
			$url = WPCardzNetLibMigratePHPClass::Safe_substr($url, WPCardzNetLibMigratePHPClass::Safe_strpos($url, '://')+3);
			return $url;
		}

		static function GetTimeFormat()
		{
			if (defined('WPCARDZNETLIB_TIME_BOXOFFICE_FORMAT'))
				$timeFormat = WPCARDZNETLIB_TIME_BOXOFFICE_FORMAT;
			else
		...
		{
			if (defined('WPCARDZNETLIB_DATE_BOXOFFICE_FORMAT'))
				$dateFormat = WPCARDZNETLIB_DATE_BOXOFFICE_FORMAT;
			else
		...
		{
			if (defined('WPCARDZNETLIB_DATETIME_BOXOFFICE_FORMAT'))
				$dateFormat = WPCARDZNETLIB_DATETIME_BOXOFFICE_FORMAT;
			else
		...
		static function GetLocalDateTime($timestamp = 0)
		{
			...
			$localTime = date(WPCardzNetLibDBaseClass::MYSQL_DATETIME_FORMAT, $timestamp);
			return $localTime;
		}
		...
		function AddFieldsToTemplate($dbRecord, $mailTemplate, &$EMailSubject, &$emailContent)
		{
			...
			$posnPHP = WPCardzNetLibMigratePHPClass::Safe_stripos($mailTemplate, '<?php');
			if ($posnPHP !== false) $posnPHP = WPCardzNetLibMigratePHPClass::Safe_strpos($mailTemplate, "\n", $posnPHP);
			if ($posnPHP !== false) $posnEOL = WPCardzNetLibMigratePHPClass::Safe_strpos($mailTemplate, "\n", $posnPHP+1);
			if (($posnPHP !== false) && ($posnEOL !== false))
			{
				$EMailSubject = $this->AddEventToTemplate(WPCardzNetLibMigratePHPClass::Safe_substr($mailTemplate, $posnPHP+1, $posnEOL-$posnPHP-1), $dbRecord[0]);
				$mailTemplate = WPCardzNetLibMigratePHPClass::Safe_substr($mailTemplate, $posnEOL);
			}

			// Find the line with the close php entry then find the start of the line
			$posnPHP = WPCardzNetLibMigratePHPClass::Safe_stripos($mailTemplate, '?>');
			if ($posnPHP !== false) $posnPHP = WPCardzNetLibMigratePHPClass::Safe_strrpos(WPCardzNetLibMigratePHPClass::Safe_substr($mailTemplate, 0, $posnPHP), "\n");
			if ($posnPHP !== false) $mailTemplate = WPCardzNetLibMigratePHPClass::Safe_substr($mailTemplate, 0, $posnPHP);
			...
			for (; $loopCount < 10; $loopCount++)
			{
				...
				$loopStart = WPCardzNetLibMigratePHPClass::Safe_stripos($mailTemplate, $matches[0]);
				$loopEnd = WPCardzNetLibMigratePHPClass::Safe_stripos($mailTemplate, '[endloop]');
				if (($loopStart === false) || ($loopEnd === false))
					break;

				$beforeLoop = WPCardzNetLibMigratePHPClass::Safe_substr($mailTemplate, 0, $loopStart);

				$loopStart += WPCardzNetLibMigratePHPClass::Safe_strlen($matches[0]);
				$loopLen = $loopEnd - $loopStart;

				$loopSection = WPCardzNetLibMigratePHPClass::Safe_substr($mailTemplate, $loopStart, $loopLen);

				$loopEnd += WPCardzNetLibMigratePHPClass::Safe_strlen('[endloop]');
				$afterLoop = WPCardzNetLibMigratePHPClass::Safe_substr($mailTemplate, $loopEnd);
		...
		function DoTemplateConditionals(&$EMailTemplate, $event)
		{
			...
			$if_len = WPCardzNetLibMigratePHPClass::Safe_strlen($if_text);
			$ifnot_len = WPCardzNetLibMigratePHPClass::Safe_strlen($ifnot_text);
			$endif_len = WPCardzNetLibMigratePHPClass::Safe_strlen($endif_text);
			$else_len = WPCardzNetLibMigratePHPClass::Safe_strlen($else_text);
			...
			while(true)
			{
				...
				$nextPosn = WPCardzNetLibMigratePHPClass::Safe_strrpos($EMailTemplate, $if_marker, $offset);
				if ($nextPosn === false) break;
					
				// Determine type of if statement 
				if (WPCardzNetLibMigratePHPClass::Safe_substr($EMailTemplate, $nextPosn, $if_len) === $if_text)
				{
					$ifTagStartPosn = $nextPosn + $if_len;
					$dbCond = true;
				}
				else if (WPCardzNetLibMigratePHPClass::Safe_substr($EMailTemplate, $nextPosn, $ifnot_len) === $ifnot_text)
				{
				...
				{
					$offset = $nextPosn - WPCardzNetLibMigratePHPClass::Safe_strlen($EMailTemplate) - 1;
					continue;
				}
				
				$TemplateLen = WPCardzNetLibMigratePHPClass::Safe_strlen($EMailTemplate);
				
				$ifTagEndPosn = WPCardzNetLibMigratePHPClass::Safe_strpos($EMailTemplate, ']', $ifTagStartPosn);
				$dbField = WPCardzNetLibMigratePHPClass::Safe_substr($EMailTemplate, $ifTagStartPosn, $ifTagEndPosn-$ifTagStartPosn);

				// Search for end marker
				$sectionEnd = WPCardzNetLibMigratePHPClass::Safe_strpos($EMailTemplate, $endif_text, $nextPosn);
				if ($sectionEnd === false) 
					return __('Missing Conditional end marker', 'wpcardznet');
				
				// Search forwards for else marker (assumes later else markers have been processed)
				$elseStart = WPCardzNetLibMigratePHPClass::Safe_strpos($EMailTemplate, $else_text, $nextPosn);
				if ($elseStart === false)
				{
				...
				if (isset($event->$dbField))
				{
					...
					{
						$dbSet = (WPCardzNetLibMigratePHPClass::Safe_strlen($event->$dbField) != 0);
					}
				}

				$startText = WPCardzNetLibMigratePHPClass::Safe_substr($EMailTemplate, 0, $nextPosn);
				$endText = WPCardzNetLibMigratePHPClass::Safe_substr($EMailTemplate, $endPosn, $TemplateLen);
				
				if ($dbSet == $dbCond)
				{
					// Replace the section including the conditional commands with the matched section
					$midText = WPCardzNetLibMigratePHPClass::Safe_substr($EMailTemplate, $ifTrueStart, $ifTrueEnd-$ifTrueStart);
				}
				else
				{
					// Replace the section with the else section (if it exists)
					$midText = WPCardzNetLibMigratePHPClass::Safe_substr($EMailTemplate, $ifFalseStart, $ifFalseEnd-$ifFalseStart);
				}

				$EMailTemplate = $startText.$midText.$endText;
				$changes++;
						
				$offset = 0 - WPCardzNetLibMigratePHPClass::Safe_strlen($midText) - WPCardzNetLibMigratePHPClass::Safe_strlen($endText);
			}
		...
		function AddEventToTemplate($EMailTemplate, $event)
		{
			foreach ($event as $key => $value)
			{
				...
				$EMailTemplate = WPCardzNetLibMigratePHPClass::Safe_str_replace($tag, $value, $EMailTemplate);
			}

			$EMailTemplate = $this->DoEmbeddedImage($EMailTemplate, 'logoimg', WPCARDZNETLIB_LOGOIMAGE_OPTIONID);
		...
		function ReadTemplateFile($Filepath)
		{
			...
			{
				WPCardzNetLibEscapingClass::Safe_EchoHTML("Error reading $Filepath<br>\n");
				$fileContents = '';
			}
		...
		function ReadEMailTemplateFile($Filepath)
		{
			...
			$this->mailTemplate_origLen = WPCardzNetLibMigratePHPClass::Safe_strlen($template);
			$template = preg_replace("/^(\..*)/m", "  $1", $template);
			$this->mailTemplate_newLen = WPCardzNetLibMigratePHPClass::Safe_strlen($template);

			if ( ($this->mailTemplate_origLen != $this->mailTemplate_newLen)
			  && (WPCardzNetLibUtilsClass::IsElementSet('post', 'EMailSale_DebugEnabled')) )
			{
				WPCardzNetLibEscapingClass::Safe_EchoHTML("************************************************************************<br>\n");
				WPCardzNetLibEscapingClass::Safe_EchoHTML(__("WARNING: EMail Template contains one or mores lines with leading dots ('.')", 'wpcardznet')."<br>\n");
				WPCardzNetLibEscapingClass::Safe_EchoHTML("************************************************************************<br><br>\n");
			}
		...
		function AddRecordToTemplate($dbRecord, $templatePath, &$EMailSubject, &$emailContent)
		{
			...
			if (WPCardzNetLibMigratePHPClass::Safe_strlen($mailTemplate) == 0)
				return "EMail Template Not Found ($templatePath)";

			$posnEndPHP = WPCardzNetLibMigratePHPClass::Safe_stripos($mailTemplate, '?>');
			$templateFooter = WPCardzNetLibMigratePHPClass::Safe_substr($mailTemplate, $posnEndPHP);
			$footerLines = explode("\n", $templateFooter);
			foreach ($footerLines as $footerLine)
			{
				...
				if (count($fields) > 1)
				{
					...
					{
						case 'Attach':
							$filename = WPCardzNetLibMigratePHPClass::Safe_trim($fields[1]);
			...
			$emailContent = apply_filters('wpcardznet'.'_filter_emailbody', $emailContent, $dbRecord[0]);
			return $status;
		}
		...
		function SendEMailByTemplateID($eventRecord, $templateID, $folder, $EMailTo = '')
		{
			...
			$templateRoot = WPCARDZNETLIB_UPLOADS_PATH.'/'.$folder.'/';
		...
		function WriteCommsLog($logMessage, $addTimestamp = false, $mode = WPCardzNetLibLogFileClass::ForAppending)
		{
			...
			if (!isset($this->logFileObj))
			{
				...
				$this->logFileObj->LogToFile(WPCARDZNETLIB_FILENAME_COMMSLOG, '', $mode);
			...
			$this->logFileObj->LogToFile(WPCARDZNETLIB_FILENAME_COMMSLOG, $logMessage, $mode);
		}
		...
		{
			$LogsFolder = ABSPATH.$this->getOption('LogsFolderPath');
			$logsPath = $LogsFolder.'/'.WPCARDZNETLIB_FILENAME_COMMSLOG;
		...
		function GetSessionCookie()
		{
			if (!isset($this->sessionCookieID))
			{
				...
				{
					$this->lastSessionErr = WPCARDZNETLIB_SESSIONERR_FALSE;
					$this->sessionCookieID = null;
				}
				else if ($this->sessionCookieID == '')
				{
					$this->lastSessionErr = WPCARDZNETLIB_SESSIONERR_INACTIVE;
					$this->sessionCookieID = null;
				}
			}
		...
		function GetSessionID()
		{
			...
			if (!isset($this->sessionTableChecked))
			{
				...
				else if (!$this->tableExists($this->DBTables->Sessions))
				{
					...
					$addSession = true;
					$this->lastSessionErr = WPCARDZNETLIB_SESSIONERR_NOTABLE;
				}
				else
				{
					$this->PurgeSessions();
					...
					{
						$addSession = true;
						$this->lastSessionErr = WPCARDZNETLIB_SESSIONERR_NOMATCH;
					}
				}
				...
				if ($addSession)
				{
					...
					$sessionExpires = date(WPCardzNetLibDBaseClass::MYSQL_DATETIME_FORMAT, $ts + WPCARDZNETLIB_SESSION_TIMEOUT);
					
					$this->sessionVal = array();
					//$this->sessionVal[WPCARDZNETLIB_SESSION_VALIDATOR] = true;
					$sessionValSer = WPCardzNetLibMigratePHPClass::Safe_str_replace('"', '\"', serialize($this->sessionVal));
		...
		function PurgeSessions()
		{
			...
			$currDateTime = date(WPCardzNetLibDBaseClass::MYSQL_DATETIME_FORMAT, current_time( 'timestamp' ));
		...
		function WriteSession()
		{
			...
			$sessionExpires = date(WPCardzNetLibDBaseClass::MYSQL_DATETIME_FORMAT, current_time( 'timestamp' ) + WPCARDZNETLIB_SESSION_TIMEOUT);
		...
		function Output_PluginHelp($exHelpHTML = '')
		{
			...
			if ($timezone == '')
			{
				...
				$statusMsg = __('Timezone not set - Set it', 'wpcardznet')." <a href=$settingsPageURL>".__('Here', 'wpcardznet').'</a>';
				WPCardzNetLibEscapingClass::Safe_EchoHTML('<div id="message" class="error"><p>'.$statusMsg.'</p></div>');
			}
			
			$version = (defined('WPCARDZNETLIB_SCREENSHOTSMODE')) ? '*.*' : $this->get_version();
			$phpinfoURL = WPCARDZNETLIB_URL.'lib/phpinfo.php';
			
			WPCardzNetLibEscapingClass::Safe_EchoHTML('<strong>'.__('Plugin', 'wpcardznet').':</strong> '.$this->get_pluginName()."<br>\n");			
			WPCardzNetLibEscapingClass::Safe_EchoHTML('<strong>'.__('Version', 'wpcardznet').':</strong> '.$version."<br>\n");			

			if ($exHelpHTML != '') WPCardzNetLibEscapingClass::Safe_EchoHTML($exHelpHTML);
				
			WPCardzNetLibEscapingClass::Safe_EchoHTML('<strong>WP:</strong> '.$this->GetWPVersion()."<br>\n");
			WPCardzNetLibEscapingClass::Safe_EchoHTML('<strong>MySQL:</strong> '.$this->GetMySQLVersion()."<br>\n");
			WPCardzNetLibEscapingClass::Safe_EchoHTML('<strong>PHP: </strong><a href="'.$phpinfoURL.'" target="_blank">'.PHP_VERSION."</a><br>\n");			
			WPCardzNetLibEscapingClass::Safe_EchoHTML('<strong>'.__('Timezone', 'wpcardznet').':</strong> '.$timezone."<br>\n");			
