...
if (!class_exists('WPCardzNetLibUtilsClass'))
{
	if (!defined('WPCARDZNETLIB_PHPFOLDER_PERMS'))
		define('WPCARDZNETLIB_PHPFOLDER_PERMS', 0755);

	if (!defined('WPCARDZNETLIB_PHPFILE_PERMS'))
		define('WPCARDZNETLIB_PHPFILE_PERMS', 0644);

	if (!defined('WPCARDZNETLIB_LOGFOLDER_PERMS'))
		define('WPCARDZNETLIB_LOGFOLDER_PERMS', 0700);

	if (!defined('WPCARDZNETLIB_CALLBACK_BASENAME'))
		define('WPCARDZNETLIB_CALLBACK_BASENAME', basename(dirname(dirname(__FILE__))).'_callback');

	class WPCardzNetLibUtilsClass // Define class
	{
		static function GetSiteID()
		{
			$siteURL = get_option('siteurl');
			$slashPosn = WPCardzNetLibMigratePHPClass::Safe_strrpos($siteURL, '/');
			$siteURL = WPCardzNetLibMigratePHPClass::Safe_substr($siteURL, $slashPosn+1);
		...
		static function GetHTTPTextElem($reqArray, $elementId = 'undef', $defaultValue = '')
		{
			...
			$str = WPCardzNetLibMigratePHPClass::Safe_stripslashes($str);
			return $str;
		}
		...
		static function wpcardznetlib_filterHTMLStyles($styles)
		{
		...
		{
			global $wpcardznetlib_allowedProtocols;

			if (!is_array($wpcardznetlib_allowedProtocols))
			{
				$wpcardznetlib_allowedProtocols = wp_allowed_protocols();
				$wpcardznetlib_allowedProtocols[] = 'data';
			}
			
			return $wpcardznetlib_allowedProtocols;
			
		}

		static function GetAllowedHTMLTags($extraTags = array())
		{
			global $wpcardznetlib_allowedHTML;

			if (!is_array($wpcardznetlib_allowedHTML))
			{
				add_filter( 'safe_style_css', array('WPCardzNetLibUtilsClass', 'wpcardznetlib_filterHTMLStyles'));
				...
				$wpcardznetlib_allowedHTML = $kses_allowed;
			}

			return $wpcardznetlib_allowedHTML;
		}
		...
		static function GetHTTPJSONEncodedElem($reqArray, $elementId)
		{
			...
			foreach($postVarsArray as $postKey => $postVal)
			{
				...
				WPCardzNetLibUtilsClass::SetElement('post', $postKey, $postVal);
				WPCardzNetLibUtilsClass::SetElement('request', $postKey, $postVal);
			}
		...
		static function GetHTTPInteger($reqArray, $elementId = 'undef', $defaultValue = null)
		{
			...
			$strval = WPCardzNetLibMigratePHPClass::Safe_trim(self::GetArrayElement($reqArray, $elementId, $defaultValue));
			$rtnval = (int)$strval;
			if ($rtnval != $strval)
			{
				return $defaultValue;
		...
		{
		    return (bool)WPCardzNetLibMigratePHPClass::Safe_strtotime($myDateString);
		}

		static function GetDateTimestamp($myDateString)
		{
			$spPosn = WPCardzNetLibMigratePHPClass::Safe_strpos($myDateString, " ");
			$ts = WPCardzNetLibMigratePHPClass::Safe_strtotime(WPCardzNetLibMigratePHPClass::Safe_substr($myDateString, 0, $spPosn));
			return $ts;
		}
		...
		static function GetPageBaseURL()
		{
			...
			$posnParams = WPCardzNetLibMigratePHPClass::Safe_strpos($pageURL,'?');
			if ($posnParams !== false)
				$pageURL = WPCardzNetLibMigratePHPClass::Safe_substr($pageURL, 0, $posnParams);
			return $pageURL;
		}
		...
		{
			return self::GetPageURL().'&'.WPCARDZNETLIB_CALLBACK_BASENAME."={$callbackFilename}";
		}
		...
		{
		     $length = WPCardzNetLibMigratePHPClass::Safe_strlen($needle);
		     return (WPCardzNetLibMigratePHPClass::Safe_substr($haystack, 0, $length) === $needle);
		}

		static function recurse_copy($src, $dst, $perm=WPCARDZNETLIB_PHPFOLDER_PERMS)
		{
			$rtnStatus = true;
		...
		static function UndefinedFuncCallError($classObj, $funcName)
		{
			...
			WPCardzNetLibEscapingClass::Safe_EchoHTML("<strong><br>function $funcName() must be defined in $classId class<br></strong>\n");
			die;
		}
		...
		static function UserMessage($msg, $isError = true, $msgClass = '')
		{
			...
			WPCardzNetLibEscapingClass::Safe_EchoHTML('<div id="message" class="'.$msgClass.'"><p>'.$msg.'</p></div>');
		}
