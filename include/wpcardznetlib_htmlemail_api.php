...
if (!class_exists('WPCardzNetLibHTMLEMailAPIClass')) 
	{
	
	define('WPCARDZNETLIB_FILENAME_EMAILLOG', 'EMailLog.log');
	class WPCardzNetLibHTMLEMailAPIClass // Define class
	{	
		var $fileobjs = array();
		var $ourContentType = '';
		...
		function __construct($ourParentObj)	
		{
			...
			$this->createMIMEmessages = ($this->mimeEncodingMode == WPCARDZNETLIB_MIMEENCODING_PLUGIN);
		...
		function createPHPMailerObj($DebugEMail, $EMailLogPath)
		{
			...
			if ($DebugEMail)
			{
				WPCardzNetLibEscapingClass::Safe_EchoHTML("Mailer Class = $mailerClass <br>\n");
				WPCardzNetLibEscapingClass::Safe_EchoHTML('Mime Encoding Mode: '."{$this->mimeEncodingMode} <br>\n");
			}
			
			if (WPCardzNetLibMigratePHPClass::Safe_strpos($mailerClass, 'PHPMailer') === false)
			{
		  		if (!$this->createMIMEmessages)
		  		{
		  			...
		  			if ($missingMethods != '')
		  			{
		  				...
		  				WPCardzNetLibEscapingClass::Safe_EchoHTML(sprintf(__('Mailer class (%s) has missing methods', 'wpcardznet'), $mailerClass)." ($missingMethods) <br>\n");
		  				WPCardzNetLibEscapingClass::Safe_EchoHTML(__('Any email attachments will be missing', 'wpcardznet')." <br>\n");
		  				WPCardzNetLibEscapingClass::Safe_EchoHTML("************************************* <br>\n");
		  			}
		  		}
		...
		function html2text($html)
		{
			...
			$bodyPosn = WPCardzNetLibMigratePHPClass::Safe_strpos($html, '<body');
			if ($bodyPosn > 0) $text = WPCardzNetLibMigratePHPClass::Safe_substr($html, $bodyPosn);
			...
			for ($i=0; $i<$noOfMatches; $i++)
			{
				...
				$origURL = WPCardzNetLibMigratePHPClass::Safe_str_replace('"', '', $origURL);
				$origURL = WPCardzNetLibMigratePHPClass::Safe_str_replace('mailto:', '', $origURL);
				...
				$text = WPCardzNetLibMigratePHPClass::Safe_str_replace($origLink, $targetText, $text);
			}
			
			$text = WPCardzNetLibMigratePHPClass::Safe_htmlspecialchars_decode($text);
		...
		function sendMail($to, $from, $subject, $content, $headers = '')
		{
			...
			if (WPCardzNetLibMigratePHPClass::Safe_strlen($headers) > 0) $headers .= "\r\n";
			$headers .= "MIME-Version: 1.0";
								
	  		// FUNCTIONALITY: EMail - Send MIME format EMail with text and HTML versions
			if ((WPCardzNetLibMigratePHPClass::Safe_strlen($content) > 0) && (WPCardzNetLibMigratePHPClass::Safe_stripos($content, '<html>') !== false))
			{
				...
				if (current_user_can(WPCARDZNETLIB_CAPABILITY_SYSADMIN)) 
				{
					...
					{
						WPCardzNetLibEscapingClass::Safe_EchoHTML("WARNING: Text Content has one or more lines longer than SMTP allows<br>\n\n\n");
					}
					
					if ($phpmailer->hasLineLongerThanMax($this->contentHTML))
					{
						WPCardzNetLibEscapingClass::Safe_EchoHTML("WARNING: HTML Content has one or more lines longer than SMTP allows<br>\n\n\n");
					}					
				}
			...
			if ($this->DebugEMail)
			{
				...
				$html_email = "To:<br>\n";
				$html_email .= WPCardzNetLibMigratePHPClass::Safe_htmlspecialchars($to);
				$html_email .= "<br>\n<br>\n";
				$html_email .= "Subject:<br>\n";
				$html_email .= WPCardzNetLibMigratePHPClass::Safe_htmlspecialchars($subject);
				$html_email .= "<br>\n<br>\n";
				$html_email .= "Headers:<br>\n";
				$html_email .= WPCardzNetLibMigratePHPClass::Safe_str_replace("\r\n", "<br>\r\n", WPCardzNetLibMigratePHPClass::Safe_htmlspecialchars($headers));
				$html_email .= "<br>\n<br>\n";
/*
				$html_email .= "Message:<br>\n";
				$html_email .= WPCardzNetLibMigratePHPClass::Safe_htmlspecialchars($message);
				$html_email .= "<br>\n<br>\n";
*/
				WPCardzNetLibEscapingClass::Safe_EchoHTML($html_email);
			}
					
			$bracket_pos = WPCardzNetLibMigratePHPClass::Safe_strpos( $from, '<' );
			if ( $bracket_pos !== false ) 
				{
				// Text before the bracketed email is the "From" name.
				if ( $bracket_pos > 0 ) 
				{
					$from_name = WPCardzNetLibMigratePHPClass::Safe_substr( $from, 0, $bracket_pos - 1 );
					$from_name = WPCardzNetLibMigratePHPClass::Safe_str_replace( '"', '', $from_name );
					$from_name = WPCardzNetLibMigratePHPClass::Safe_trim( $from_name );
				}

				$from_email = WPCardzNetLibMigratePHPClass::Safe_substr( $from, $bracket_pos + 1 );
				$from_email = WPCardzNetLibMigratePHPClass::Safe_str_replace( '>', '', $from_email );
				$from_email = WPCardzNetLibMigratePHPClass::Safe_trim( $from_email );
			} 
			else
			{
				$from_email = WPCardzNetLibMigratePHPClass::Safe_trim( $from );
				$from_name = '';
			}

			$this->WPCardzNetLib_From = $from_email;
			$this->WPCardzNetLib_FromName = $from_name;
			...
			{
				if ($this->DebugEMail)
					WPCardzNetLibEscapingClass::Safe_EchoHTML(("Calling wp_mail <br>\n"));
				...
				{
					if ($this->DebugEMail)
						WPCardzNetLibEscapingClass::Safe_EchoHTML(("Deleting temporary file: $filePath <br>\n"));
					unlink($filePath);
				}
			}
			else
			{
				WPCardzNetLibEscapingClass::Safe_EchoHTML("<br><strong>Sending of EMails Blocked </strong><br><br>\n");
			}
		...
		{
			$fileTypePosn = WPCardzNetLibMigratePHPClass::Safe_strripos($filePath, '.');
			if (!$fileTypePosn) return;
				
			$fileExtn = WPCardzNetLibMigratePHPClass::Safe_strtolower(WPCardzNetLibMigratePHPClass::Safe_substr($filePath, $fileTypePosn+1));
		...
		function OutputMIMEFile($fileobj)
		{
			...
			if (WPCardzNetLibMigratePHPClass::Safe_strlen($bin) == 0) return;
		...
		function FixPHPMailer($phpmailer)
		{
			...
			*/
			...
			if (WPCardzNetLibMigratePHPClass::Safe_strpos($mailMIME, 'Content-Type') === false)
				return;
			...
			foreach ($customHeaders as $customHeader)
			{
				...
				{
					if ($this->DebugEMail)
						WPCardzNetLibEscapingClass::Safe_EchoHTML((__('Duplicate Content-Type header removed from PHPMailer', 'wpcardznet')."<br>\n"));
					continue;
				}
		...
		function AddFilesToEMail($phpmailer)
		{
			...
			foreach ($this->fileobjs as $fileobj)
			{
				...
				if ($path != '')
				{
					...
					{					
						if ($this->DebugEMail)
							WPCardzNetLibEscapingClass::Safe_EchoHTML("Calling addEmbeddedImage($file) <br>\n");
						$phpmailer->addEmbeddedImage($path, $cid, $file, $encoding, $type, $disposition);        
					}
					else
					{
						if ($this->DebugEMail)
							WPCardzNetLibEscapingClass::Safe_EchoHTML("Calling addAttachment($file) <br>\n");
				...
				else 
				{
					// Data included in object
					{
						...
						if ($this->DebugEMail)
							WPCardzNetLibEscapingClass::Safe_EchoHTML(("Adding image to temporary file: $imgFilePath <br>\n"));
		...
		{
			$mailFrom = $this->WPCardzNetLib_From;		
			return $mailFrom;
		}
		
		function FilterMailFromName($mailFromName)
		{
			$mailFromName = $this->WPCardzNetLib_FromName;		
			return $mailFromName;
		}
		...
		{ 
			if ($this->DebugEMail)
			{
				WPCardzNetLibEscapingClass::Safe_EchoHTML(("wp_mail response: wp_mail_succeeded <br>\n"));
			}
		...
		function Get_EMailErrors($excp)
		{
			...
			if ($this->DebugEMail)
			{
				$htmlErrMsg = WPCardzNetLibMigratePHPClass::Safe_htmlspecialchars($errMsg);
				WPCardzNetLibEscapingClass::Safe_EchoHTML(("wp_mail response: wp_mail_failed <br>\n"));
				WPCardzNetLibEscapingClass::Safe_EchoHTML(("error msg: $htmlErrMsg <br>\n"));
			}
		}
